...
@Singleton
public class ModelTabController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        messageBinders = new MessageBindersBuilder()
                ...
                .bind("brand_id").to(brandComboBox)
                .build();
    ...
    public void find(ActionEvent event) {
        try {
            ObservableList<PersistentObject> result = modelService.findModelList();
    ...
    protected void edit(ActionEvent event) {
        ...
        PersistentObject model = tableView.getSelectionModel().getSelectedItem();

        try {
            model = modelService.findModelById((Integer) model.getProperty("id"));
        } catch (Exception e) {
            e.printStackTrace();
            return;
        }

        idTextField.setText(model.getProperty("id").toString());
        nameTextField.setText(model.getProperty("name").toString());
        ...
        // select brand
        brandComboBox.getSelectionModel().clearSelection();
        int brandId = (int) model.getProperty("brand_id");
        ...
        try {
            ...
            switch (formState) {
                ...
                case Edit:
                    ...
                    try {
                        item = modelService.findModelById((Integer) item.getProperty("id"));
                    } catch (Exception e) {
                        e.printStackTrace();
                        return;
                    }
            ...
            if (container.validate(new ModelValidator(), formPane, messageBinders, item)) {
                modelService.saveModel(item);
                switch (formState) {
                    case Add:
                        items.add(item);
                        break;
    ...
    private void populateData(PersistentObject po) {
        ...
        if (brandComboBox.getSelectionModel().getSelectedItem() != null) {
            brandId = (Integer) brandComboBox.getSelectionModel().getSelectedItem().getProperty("id");
        }

        modelService.populateModel(po, name, brandId);
