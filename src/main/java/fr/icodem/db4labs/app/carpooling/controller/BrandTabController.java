...
@Singleton
public class BrandTabController implements Initializable {
    ...
    public void find(ActionEvent event) {
        try {
            ObservableList<PersistentObject> result = brandService.findBrandList();
    ...
    protected void edit(ActionEvent event) {
        ...
        PersistentObject brand = tableView.getSelectionModel().getSelectedItem();

        try {
            brand = brandService.findBrandById((Integer) brand.getProperty("id"));
        } catch (Exception e) {
            e.printStackTrace();
            return;
        }

        idTextField.setText(brand.getProperty("id").toString());
        nameTextField.setText(brand.getProperty("name").toString());
        showForm();
    }
    ...
    protected void save(ActionEvent event) {
        ...
        try {
            ...
            switch (formState) {
                case Add:
                    // populate data
                    item = new PersistentObject("brand");
                    break;
                case Edit:
                    // populate data
                    item = tableView.getSelectionModel().getSelectedItem();
                    try {
                        item = brandService.findBrandById((Integer) item.getProperty("id"));
                    } catch (Exception e) {
                        e.printStackTrace();
                        return;
                    }
            ...
            if (container.validate(new BrandValidator(), formPane, messageBinders, item)) {
                brandService.saveBrand(item);
                switch (formState) {
                    case Add:
                        items.add(item);
                        break;
    ...
    @Subscribe
    public void brandAdded(BrandAddedEvent event) {
        // update table
        Platform.runLater(() -> {
            ObservableList<PersistentObject> items = tableView.getItems();
            items.add(event.getBrand());
        });
    }
