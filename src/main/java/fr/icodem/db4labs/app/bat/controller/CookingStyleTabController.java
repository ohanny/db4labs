...
@Singleton
public class CookingStyleTabController implements Initializable {
    ...
    public void find(ActionEvent event) {
        try {
            ObservableList<PersistentObject> result = cookingStyleService.findCookingStyleList();
    ...
    protected void edit(ActionEvent event) {
        ...
        try {
            tag = cookingStyleService.findCookingStyleById((Integer) tag.getProperty("id"));
        } catch (Exception e) {
            e.printStackTrace();
            return;
        }
    ...
    protected void save(ActionEvent event) {
        ...
        try {
            ...
            switch (formState) {
                case Add:
                    // populate data
                    item = new PersistentObject("cooking_style");
                    break;
                case Edit:
                    // populate data
                    item = tableView.getSelectionModel().getSelectedItem();
                    try {
                        item = cookingStyleService.findCookingStyleById((Integer) item.getProperty("id"));
                    } catch (Exception e) {
                        e.printStackTrace();
                        return;
                    }
            ...
            if (container.validate(new CookingStyleValidator(), formPane, messageBinders, item)) {
                cookingStyleService.saveCookingStyle(item);
                switch (formState) {
                    case Add:
                        items.add(item);
                        break;
    ...
    protected void delete(ActionEvent event) {
        ...
        try {
            cookingStyleService.deleteCookingStyle(po);
